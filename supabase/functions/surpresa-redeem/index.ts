...
serve(async (req: Request) => {
  ...
  try {
    ...
    if (reward.status === "redeemed") {
      return new Response(
        JSON.stringify({
          ...
          message: "Este prêmio já foi resgatado.",
    ...
    if (reward.status === "cancelled") {
      return new Response(
        JSON.stringify({ error: "cancelled", message: "Este prêmio foi cancelado." }),
        { status: 400, headers: { ...corsHeaders, "Content-Type": "application/json" } }
      );
    }
    ...
    // Only validated rewards can be redeemed (unless share is not required)
    if (reward.status !== "validated" && reward.status !== "pending_share" && reward.status !== "pending_validation") {
      return new Response(
        JSON.stringify({
          error: "invalid_status",
          message: `Status atual: ${reward.status}. O prêmio precisa estar validado para ser resgatado.`,
        }),
        { status: 400, headers: { ...corsHeaders, "Content-Type": "application/json" } }
      );
    }
    ...
    if (updateError) {
      return new Response(
        JSON.stringify({ error: "update_failed", message: "Erro ao resgatar." }),
    ...
    await supabase.from("campaign_events").insert({
      ...
      metadata: { redeemed_by: user.id, reward_code: code },
    });
    ...
    return new Response(
      JSON.stringify({
        ...
        participant_name: reward.participation.participant_name,
      }),
