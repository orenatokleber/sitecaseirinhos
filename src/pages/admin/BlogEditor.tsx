...
const BlogEditor = () => {
  ...
  const handleSave = async (publish = false) => {
    ...
    try {
      if (isEditing) {
        await updatePost.mutateAsync({ id: id!, updates: payload });
      } else {
        await createPost.mutateAsync(payload);
      }
      setHasChanges(false);
      if (!isEditing) {
        navigate("/painel-admin/blog");
      }
    } finally {
      setIsSaving(false);
    }
            ...
            <div className="p-4 space-y-5">
                    ...
                    <div className="flex items-center justify-between p-3 bg-muted/50 rounded-lg">
                      <div>
                        ...
                        <p className="text-xs text-muted-foreground">
                          {formData.is_published ? "Visível no site" : "Apenas você pode ver"}
                        </p>
                      </div>
                      <Switch
              ...
              {sidebarTab === "block" && (
                <div className="text-center py-8 text-muted-foreground">
                  <p className="text-sm">Selecione um bloco no editor para ver suas configurações</p>
                </div>
              )}
            </div>
          </ScrollArea>
