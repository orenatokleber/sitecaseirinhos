        ...
        <div className="container mx-auto px-4 max-w-5xl">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-12 items-center mb-20">
            <motion.div initial={{ opacity: 0, x: -30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.7 }}>
              <img src={confeiteiraSorrindo} alt="Confeiteira Caseirinhos com fouet" className="rounded-lg object-cover object-[center_25%] w-full h-[420px] shadow-md" loading="lazy" decoding="async" />
            </motion.div>
            <motion.div initial={{ opacity: 0, x: 30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.7 }} className="space-y-6 text-muted-foreground leading-relaxed">
              ...
              <p>Tudo começou com uma paixão simples: a vontade de fazer bolos que trouxessem sorrisos. Em uma pequena cozinha, com receitas de família e muita dedicação, nasceu a Caseirinhos — uma confeitaria artesanal que acredita que cada doce conta uma história.</p>
              <p>O que começou com bolos caseiros para amigos e familiares se transformou em algo maior: bolos de casamento que emocionam, sobremesas que surpreendem, doces finos que encantam. Cada encomenda é tratada com o mesmo carinho do primeiro bolo.</p>
            </motion.div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-12 items-center">
            <motion.div initial={{ opacity: 0, x: -30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.7 }} className="space-y-6 text-muted-foreground leading-relaxed md:order-1 order-2">
              <p>Hoje, a Caseirinhos é mais do que uma confeitaria. É um pedaço de afeto em forma de doce. É a certeza de que cada ingrediente foi escolhido com cuidado, cada decoração foi pensada com atenção, e cada entrega é feita com orgulho.</p>
            ...
            <motion.div initial={{ opacity: 0, x: 30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.7 }} className="md:order-2 order-1">
              <img src={decorandoBolo} alt="Confeiteira decorando bolo artesanal" className="rounded-lg object-cover object-[center_30%] w-full h-[420px] shadow-md" loading="lazy" decoding="async" />
            </motion.div>
          </div>
        </div>
      </section>
        ...
        <div className="container mx-auto px-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 max-w-4xl mx-auto">
            {[
              ...
              <motion.div key={v.title} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }} className="text-center p-8">
